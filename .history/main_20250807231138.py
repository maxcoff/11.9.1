...
from core.ui import run_async_ui
...
async def main():
    ...
    try:        
            # 1) стартуем таски оркестратора (например, внутри .run() он создаёт свои loops)
        orchestration_task = asyncio.create_task(orchestrator.run())

        # 2) сразу же стартуем TUI
        ui_task = asyncio.create_task(run_async_ui(orchestrator))

        # 3) ждём, что произойдёт раньше: выход из UI (по q/кнопке) или падение оркестратора
        #done, pending = await asyncio.wait(
        #    [orchestration_task, ui_task],
        #    return_when=asyncio.FIRST_COMPLETED,
        #)

        # 4) при выходе из UI — корректно завершаем оркестратор
        #if ui_task in done:
        ...
        # 5) если оркестратор упал сам — отменяем UI
        #if orchestration_task in done:
        #    ui_task.cancel()
        #    try:
        #        await ui_task
        #    except asyncio.CancelledError:
