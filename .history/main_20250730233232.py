...
async def main():
    ...
    try:
        ...
        logger.warning(
            f"⏰ Initial time sync failed: {e}",
            extra={"mode":"START","errorCode":"-"}        )
...
if __name__ == "__main__":
    use_demo = get_bool("USE_DEMO", False)    
    if use_demo: rest_url = get("REST_URL_DEMO"), api_key = get("API_KEY_DEMO"), api_secret = get("API_SECRET_DEMO"), passphrase = get("PASSPHRASE_DEMO")  # type: ignore
    else: rest_url   = get("REST_URL"), api_key = get("API_KEY"), api_secret = get("API_SECRET"), passphrase = get("PASSPHRASE")# type: ignore
    session     = aiohttp.ClientSession()
    rest_client = RestClient(api_key = api_key, api_secret = api_secret, passphrase = passphrase,session = session, base_url = rest_url, use_demo = use_demo) # type: ignore
    try:
        asyncio.run(main())
        ...
        logger.info(f" 👋 REST session closed: {res}", extra={"mode":"MAIN"})
