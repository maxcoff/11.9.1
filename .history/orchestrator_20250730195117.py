...
from tpsl_monitor      import TpslMonitor
from logger            import logger
from config            import get
from prehedge_watcher  import PreHedgeWatcher
...
class Orchestrator:    
    ...
    def __init__(self,
        ...
        self.tp_watcher     = PreHedgeWatcher(self.rest,
                                            self.ws_monitor,
                                            self.hedge_manager)
        ...
        # создаём и запускаем модуль реинвеста как отдельный компонент
        self.reinvest = Reinvest(
            rest_client=self.rest,
            ws_monitor=self.ws_monitor,
            order_client=self.order_client,
            tpsl_monitor=self.tpsl,
    ...
    async def run(self):
        ...
        self.tpsl.start()
        self._reinvest_task = asyncio.create_task(self.reinvest.run())
        ...
        # 3) initial snapshot of positions (REST)
        ...
        while self.running:
            try:
                ...
                if self.hedged and long_q == 0 and short_q == 0:
                    logger.info(
                        "→ All legs closed → back to ORDER mode",
                ...
                if not self.tpsl.is_active():
                    logger.warning("[TPSL] task not running – restarting",
                                   extra={"mode": "TPSL"})
                    self.tpsl.start()
    ...
    async def _reinvest_loop(self):
        self.logger = logger  # если ещё не импортировано        
        while self.running:
            ...
            try:
                ...
                # очищаем, чтобы ловить следующий тик
