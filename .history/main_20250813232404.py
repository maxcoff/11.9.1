...
from core.http_server import start_http_server, state as http_state
...
async def main():
    ...
    await start_http_server(orch, logger=logger, host="192.168.100.254", port=8000, admin_token="секрет")
    await orch.run()
    print("ORCH main id:", id(orch))
    print("ORCH http_state id:", id(http_state.orchestrator))
    orchestration_task: Optional[asyncio.Task] = None
    await asyncio.gather(
        orch.run(),
        asyncio.sleep(0)  # просто чтобы gather не схлопнулся, если run() мгновенно завершается
    )
    try:
