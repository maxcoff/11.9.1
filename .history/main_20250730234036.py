...
if __name__ == "__main__":
    use_demo = get_bool("USE_DEMO", False)    
    if use_demo: rest_url = get("REST_URL_DEMO"); api_key = get("API_KEY_DEMO"); api_secret = get("API_SECRET_DEMO"); passphrase = get("PASSPHRASE_DEMO")  
    else: rest_url   = get("REST_URL"); api_key = get("API_KEY"); api_secret = get("API_SECRET"); passphrase = get("PASSPHRASE")
    session     = aiohttp.ClientSession()
    ...
    try:
        ...
        logger.info(f" 👋 REST session closed", extra={"mode":"MAIN"})
