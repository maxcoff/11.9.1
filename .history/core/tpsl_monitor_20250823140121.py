...
class TpslMonitor:
    ...
    def on_fill_event(self, ev: "AlgoFillEvent") -> None:
        ...
        if not suffix.endswith("OCO"):            
            logger.debug("[TPSL] drop: is_oco=False (%s)", ev.algoClOrdId)
            return
        ...
        qty = float(fill_sz)
        logger.info(f"[TPSL] TP-OCO matched: qty={qty} hedge_side={hedge_side}")

        # 6) Запускаем реинвест не блокируя поток ws-monitor
        if qty >= self.rei_thres:
        ...
        else: logger.info(f"[TPSL] Filled TP size < reinvest treshold -> skip reinvest")
    ...
    async def _loop(self):
        ...
        try:
            while True:
                ...
                cid_init = f"{init_side[0].upper()}{inst_clean}{"OCO"}"
                cid_hedge = f"{hedge_side[0].upper()}{inst_clean}{"COND"}"
            ...
            logger.error(f"❌ [TPSL] loop error: {e}")
        finally:
            logger.info("[TPSL] off")
    ...
    async def _reinstall_all(
        ...
        try:
            ...
            logger.info(f"🎯 [TPSL] placed OCO({init_cid}) @ TP{tp_init}, SL{tp_hedge}")
        except Exception as e:
