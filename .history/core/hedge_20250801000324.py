...
class HedgeManager:
    def __init__(self, rest_client, ws_monitor, logger=logger):
        ...
        self.spike_filter_delay = get_float("SPIKE_FILTER_DELAY", 0.0)
    ...
    async def run(self) -> None:
        ...
        if initial_pos == 0:
            ...
            while True:
                ...
                ws_long, ws_short = await self._get_ws_positions()
                self.logger.debug(
                    f"[HEDGE] poll WS pos (L,S)=({ws_long:.4f},{ws_short:.4f})",
                    extra={"mode":"HEDGE"}
                )
        ...
        self.logger.info(
            f"🔍 [HEDGE] initial position detected: {initial_pos:.4f}",
            extra={"mode":"HEDGE"}
        )
        ...
        self.logger.info(
            f"🔧 [HEDGE] placing market hedge: side={side}, size={hedge_size:.4f}",
            extra={"mode":"HEDGE"}
        )
        ...
        try:
            ...
            self.logger.info(
                f"🎯 [HEDGE] placed hedge ordId={hedge_ord}",
                extra={"mode":"HEDGE"}
            )
        ...
        self.logger.info(
            "→ HEDGE.run() done, handing off to TPSL",
            extra={"mode":"HEDGE"}
        )
