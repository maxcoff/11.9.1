...
class TUIOrchestrator(App):
    ...
    async def on_mount(self):
        # Добавляем логгер только один раз
        logger.propagate = False  # отключаем передачу в root-логгер
        logger.handlers.clear()   # очищаем старые хендлеры, если есть
        logger.addHandler(UILogHandler(self._append_to_log_safe))

        # Отображаем отложенные сообщения, если были
        if self._pending_chunks:
            for s in self._pending_chunks:
                self.log_panel.append_raw(s)
            self._pending_chunks.clear()

        # Монтируем статус в нижнюю часть экрана
