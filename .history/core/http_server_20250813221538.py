...
def build_status() -> StatusOut:
    ...
    for name in ("get_status", "status", "state"):
        attr = getattr(state.orchestrator, name, None)
        try:
            if callable(attr):
                r = attr()
    ...
    return StatusOut(running=is_running(), uptime_seconds=uptime, details=details)
...
async def on_startup():
    ...
    state.logger.info("HTTP server started")
...
async def api_stop():
    ...
    res = await maybe_call(state.orchestrator, "stop")
