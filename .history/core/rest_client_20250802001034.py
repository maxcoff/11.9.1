...
class RestClient:
    def __init__(
        ...
        self.max_retries    = 4       # макс. попыток на один запрос для нестабильного соединения
        self.base_backoff   = 1.0     # базовый бэкофф в секундах
        ...
        if session is None:
            ...
            logger.info(f"HTTP session created with timeout={timeout.total} seconds in rest client", extra={"mode":"START"})
        else:
            self.session = session
            logger.info("Using externally provided HTTP session")
    ...
    async def request(
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:                
                ...
                # при третьем таймауте — перезапускаем сессию
                if attempt == 3:
                    await self._reset_session()
                # логируем попытку и кодируем бэкофф
                backoff = self.base_backoff * 2 ** (attempt - 1)
                # небольшой случайный джиттер, чтобы не совпадали параллельные таски
                jitter = random.uniform(0, backoff * 0.1)
                delay = backoff + jitter

                self.logger.error(
                    f"[TimeoutError] {method} {url}, attempt {attempt}/{self.max_retries}, "
                    f"sleeping {delay:.1f}s before retry",
                    extra={"mode":"REST","errorCode":"TIMEOUT"},
                    exc_info=e
                )
        ...
        if self.session.closed:
            raise RuntimeError("Attempt to use closed session")

        raise RuntimeError(f"All {self.max_retries} attempts failed for {url}")
    
    async def _reset_session(self):
        self.logger.warning("Restarting HTTP session after repeated timeouts")
