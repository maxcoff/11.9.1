    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                logger.debug(f"[ORCHESTRATOR] WS positions: long={long_q:.4f}, short={short_q:.4f}",extra={"mode": "ORCHESTRATOR"})
                if self.prehedge_task : a= True
                else: a = False                
                logger.debug(f"[ORCHESTRATOR] hedged={self.hedged},self.prehedge_task={a}", extra={"mode": "ORCHESTRATOR"})
                ...
                # ─── TPSL mode ───
                    ...
                    if self.prehedge_task and not self.prehedge_task.done():
                        ...
                        try:
                            ...
                            #logger.info("2→ PRE-HEDGE task cancelled", extra={"mode": "PREHEDGE"})
                            self.prehedge_task = None
                            pass  # ок, таска завершена
                        await asyncio.sleep(0)
                    self.hedged = True
                '''
                # ─── проверка и отмена PRE HEDGE watcher ───
                if self.hedged and not (long_q > 0) ^ (short_q > 0):                    
                    if self.prehedge_task :
                        logger.info("1→ PRE-HEDGE done → cancel task prehedge", extra={"mode": "PREHEDGE"})                                                        
                        self.prehedge_task.cancel()
                        try:
                            await self.prehedge_task
                        except asyncio.CancelledError:
                        ...
                        if not self.prehedge_task : logger.info("prehedge_task fully cancelled", extra={"mode":"PREHEDGE"})                        
                        if self.prehedge_task : a= True
                        else: a = False                
                        logger.debug(f"[ORCHESTRATOR] self.prehedge_task={a}", extra={"mode": "ORCHESTRATOR"})'''
