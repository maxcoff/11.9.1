...
class PreHedgeWatcher:
    ...
    def _on_ticker(self, last_px: float):
        ...
        logger.debug(
            f"[PreHEDGE] pnl={pnl:.4f}, threshold={threshold_dec:.4f}, loss_evt_set={self._loss_evt.is_set()}"
        )
        ...
        if pnl < -threshold_dec:
            if self.spike_filter_delay > 0:
                ...
                if not self._loss_evt.is_set():
                    logger.info(f"🚨 [PreHEDGE] loss trigger hit: pnl={pnl:.4f} (threshold={threshold_dec:.4f})")
                    self._loss_evt.set
