...
class Orchestrator:
    ...
    async def run(self):
        ...
        logger.debug(f"Initial REST snapshot: long_q={long_q}, short_q={short_q}")

        # точная проверка
        EPS = 1e-8
        long_nonzero = isinstance(long_q, (int, float)) and long_q > EPS
        short_nonzero = isinstance(short_q, (int, float)) and short_q > EPS

        if long_nonzero and short_nonzero:
            self.hedged = True
            logger.info("Startup detected TWO legs → enter TPSL mode", extra={"mode": "STARTUP"})
        elif long_nonzero ^ short_nonzero:
        ...
        while self.running:
            try:
                ...
                # ─── INITIAL PRE-HEDGE mode (first single leg) ───                
                    ...
                    logger.debug(
                        "CHECK PRE-HEDGE: hedged=%s, long_q=%s, short_q=%s, "
                        "task_exists=%s, task_done=%s",
                        self.hedged,
                        long_q,
                        short_q,
                        bool(self.prehedge_task),
                        getattr(self.prehedge_task, "done", lambda: None)()
                    )
