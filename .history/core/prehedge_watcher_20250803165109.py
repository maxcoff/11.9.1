...
class PreHedgeWatcher:
    ...
    def _on_ticker(self, last_px: float):
        ...
        #logger.debug(
        #    f"📊 [PreHEDGE] PnL={pnl:.4f} ({last_px} vs {self.entry_price}, self.threshold={self.threshold})"
        #)   
        # учёт шорта
        if self.side == "short":
            pnl = -pnl
        ...
        if pnl < -self.threshold:
            #debug
            print(f"[PreHEDGE] loss trigger hit: pnl={pnl:.4f} < -{self.threshold:.2%}")
            logger.info(
                f"⏱ [PreHEDGE] loss trigger hit: pnl={pnl:.4f} < -{self.threshold:.2%}",
                extra={"mode": "PREHEDGE"}
            )
            if self.spike_filter_delay > 0:
                logger.info(
                    f"⏱ [PreHEDGE] spike-filter: waiting {self.spike_filter_delay:.2f}s",
                    extra={"mode": "PREHEDGE"}
                )
