...
from typing import Optional, Dict, Tuple, Callable, List, Any, Awaitable
...
class WSMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self._ext_listeners: dict[tuple[str, str], Callable[[dict], None]] = {}
    ...
    async def _handle_message(self, msg: dict) -> None:
        ...
        key = (ch, inst_id)
        if key in self._ext_listeners:
            try:
                asyncio.create_task(self._ext_listeners[key](msg))  
            except Exception:
    ...
    def subscribe(self, channel: str, inst_id: str, callback: Callable[[dict], Awaitable[None]]) -> None:
        """
        Подписаться на любой канал (private или public) и получать
        колбэк-сообщения вида dict.
        """
        # сохраняем колбэк
        key = (channel, inst_id)
        self._ext_listeners[key] = callback
