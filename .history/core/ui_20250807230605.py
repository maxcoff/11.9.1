...
class TUIOrchestrator(App):
    ...
    # Дополнительно: ловим ошибки внутри App, если нужно
    async def on_error(self, event: events.Error) -> None:  # type: ignore
        """
        Перехватываем любое необработанное исключение внутри Textual
        и пишем подробный трейсбек в лог-панель.
        """
        # Формируем список строк с трейсбеком
        lines = traceback.format_exception(
            type(event.error), event.error, event.traceback
        )
        # Выводим каждую строку в лог-панель (без лишних '\n')
        for line in lines:
            self.log_panel.write(line.rstrip())

        # Останавливаем дальнейшую «фатальную» обработку — приложение не упадёт
        event.stop()
