...
# 7) StreamHandler (консоль) # закомменчено при переходе на виджет в textual
#console = logging.StreamHandler()
#console.setLevel(logging.DEBUG)
#console.setFormatter(formatter)
#ogger.addHandler(console)

formatter = logging.Formatter('[%(asctime)s] %(levelname)s %(name)s: %(message)s', '%d-%m %H:%M:%S')
#console = logging.StreamHandler()
#console.setFormatter(formatter)
