...
class TaskManager:
    ...
    def _ensure_loop(self):
        """
        Если цикл ещё не привязан — берём текущий активный.
        Вызывается при первом запуске задачи.
        """
        if self._loop is None:
            try:
                # если мы внутри уже работающего цикла
                self._loop = asyncio.get_running_loop()
            except RuntimeError:
                # цикла ещё нет — создаём и назначаем
