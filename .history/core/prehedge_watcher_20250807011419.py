...
class PreHedgeWatcher:
    ...
    async def _place_tp(self):
        ...
        algo_id = conditional.get("algoId") if conditional else None
        try:
            await self.rest.request(
                "POST",
                "/api/v5/trade/cancel-algos",
                params={"instId": self.inst , "algoId": algo_id}
            )
        except Exception as e:
            code = getattr(e, "code", 0)
            if code == 51000:          # «No algo order to cancel»
                logger.debug("[PreHEDGE] cancel-algos: nothing to cancel")
            else:
                logger.warning(f"[PreHEDGE] cancel-algos failed: {e}")
    ...
    def _on_ticker(self, last_px: float):
        ...
        if pnl < -self.threshold: 
            #logger.debug(f"[PreHEDGE] ticker: last_px={last_px}, entry_price={self.entry_price}, pnl={pnl:.4f}, side={self.side}, threshold={self.threshold:.4f}")
