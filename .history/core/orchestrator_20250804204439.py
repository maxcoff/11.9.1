            ...
            try:
                ...
                if not self.hedged and long_q == 0 and short_q == 0:
                    ...
                    if self.prehedge_task and not self.prehedge_task.done():
                        if self.tp_watcher.tp_placed == True:
                            self.tp_watcher.tp_placed = False   # сброс
                            logger.debug(f"[ORCHESTRATOR] tp_placed reset → ORDER mode, {self.tp_watcher.tp_placed}")
