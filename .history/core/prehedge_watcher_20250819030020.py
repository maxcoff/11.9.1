...
class PreHedgeWatcher:
    def __init__(self, rest, ws_monitor, hedge_manager):
        ...
        self.ws.subscribe('books', self.inst, self.on_orderbook)
        self.ws.subscribe('mark-price', self.inst, self.on_mark_price)


    # обработчик стакана
    def on_orderbook(self, bid, ask):
        self.best_bid_dec = Decimal(str(bid))
        self.best_ask_dec = Decimal(str(ask))

    # обработчик марк‑цены
    def on_mark_price(self, mark_price):
        self.mark_price_dec = Decimal(str(mark_price))
    ...
    async def run(self):    
        ...
        # подписки при инициализации стратегии
        self.ws.subscribe('books', self.inst, self.on_orderbook)
        self.ws.subscribe('markPx', self.inst, self.on_mark_price)
    ...
    def _on_ticker(self, last_px: float):
        ...
        mid_price_dec = None
        if self.best_bid_dec is not None and self.best_ask_dec is not None:
            mid_price_dec = (self.best_bid_dec + self.best_ask_dec) / Decimal('2')

        # универсальный PnL, отрицательный — убыток
        pnl = direction * (last_px_dec - entry_dec) / entry_dec

        logger.debug(
            "[PNL DEBUG] side=%s | entry_price=%s | last_px=%s | mark_price=%s | mid_price=%s "
            "| direction=%s | raw_ratio=%s | pnl=%s | threshold=%s",
            self.side,
            self.entry_price,
            last_px_dec,
            self.mark_price_dec,
            mid_price_dec,
            direction,
            (last_px_dec - entry_dec) / entry_dec,
            pnl,
            threshold_dec
            )
