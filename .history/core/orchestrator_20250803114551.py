    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                logger.debug(
                    f"[ORCHESTRATOR] WS positions: long={long_q:.4f}, short={short_q:.4f}, hedged={self.hedged},self.prehedge_task={self.prehedge_task},self.prehedge_task.done()={self.prehedge_task.done() if self.prehedge_task else 'N/A'}",
                    extra={"mode": "ORCHESTRATOR"}
                )
