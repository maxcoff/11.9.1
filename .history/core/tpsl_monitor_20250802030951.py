...
class TpslMonitor:
    def __init__(self, rest, ws_monitor, entry_side: str, init_cid: str):
        self.rest       = rest
        self.ws         = ws_monitor
        ...
        # entry_side — "long" или "short"
        self._entry_side: str = entry_side
        self._init_cid: str = init_cid
        self._tp_lock: asyncio.Lock = asyncio.Lock()
        

        # (entry_price, entry_qty, tp_price)
        self._last_tp: Tuple[Decimal, Decimal, Decimal] = (
            Decimal(0),
            Decimal(0),
            Decimal(0),
        )
        ...
        # аннотация Decimal вместо float
        self._last_entry_price: Decimal = Decimal(0)
        self._last_entry_qty: Decimal = Decimal(0)
        self._last_tp_price: Decimal = Decimal(0)
    ...
    def last_tp_fill(self) -> Tuple[Decimal, Decimal, Decimal]:
        """
    ...
    def on_fill_event(self, fill_event: Any) -> None:
        ...
        try:
            px = Decimal(str(getattr(fill_event, "fillPx", 0)))
            sz = Decimal(str(getattr(fill_event, "fillSz", 0)))
        ...
        side = getattr(fill_event, "side", "").upper()  # "BUY" или "SELL"

        is_entry = (
            (self._entry_side == "long" and side == "BUY")
            or (self._entry_side == "short" and side == "SELL")
        )
        is_tp = (
            (self._entry_side == "long" and side == "SELL")
            or (self._entry_side == "short" and side == "BUY")
        )

        if is_entry:
            # теперь аннотация совпадает с типом Decimal
            self._last_entry_price = px
            self._last_entry_qty = sz
            logger.debug(
                f"[TPSL] entry filled: side={side}, size={sz}@{px}",
                extra={"mode": "TPSL"},
            )
            return

        if is_tp:
            async def _save_and_fire():
                ...
                logger.info(
                    f"✅ [TPSL] TP-fill detected: size={sz}@{px}",
                    extra={"mode": "TPSL"},
                )
                self.tp_filled_evt.set()

            asyncio.get_event_loop().create_task(_save_and_fire())
            return

        # остальные филлы игнорируем
