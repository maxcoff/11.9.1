...
class Orchestrator:
    ...
    async def run(self):
        ...
        self.snapshot_task = create_tracked_task(self.snapshot.run(), "snapshot")
        print(f"Создаю задачу snapshot в loop id={id(asyncio.get_running_loop())}")
        # 2.2. Классический TPSL-цикл
        self.tpsl.start()
        # 2.3. Ре-инвест-цикл (без _pending_snapshot)
        create_tracked_task(self._reinvest_loop(),"reinvest loop")

        # 3) graceful shutdown handlers
        loop = asyncio.get_running_loop()
        def _shutdown_signal(*_):
            create_tracked_task(self._shutdown(),"shutdown")
        ...
        if long_q > 0 and short_q > 0:
            ...
            await asyncio.sleep(self.poll_interval * 3)          
            self.prehedge_task = create_tracked_task(self.tp_watcher.run(), "TP watcher")


        # 5) Основной цикл опроса
        await asyncio.sleep(self.poll_interval * 5)  # задержка для инициализации
        ...
        while self.running:
            ...
            try:                
                ...
                #print(logger.name)
                #logger.info("Проверка логгера в UI")
                ...
                if not self.hedged and long_q == 0 and short_q == 0:
                    if not self.order_task or self.order_task.done():
                        ...
                        self.order_task = create_tracked_task(self.order_client.run(),"order mode" )
    ...
    async def _reinvest_loop(self):
        while self.running:
            ...
            #await asyncio.sleep(0.5) # дать WS обновить кэш
    ...
    def get_status(self) -> dict:
        return {
            "orders_count": int(self._orders_count),
            "positions": [p.as_dict() for p in self._positions],  # или свой сериализатор
