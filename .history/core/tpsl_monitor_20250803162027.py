...
class TpslMonitor:
    def __init__(self, rest, ws_monitor):
        ...
        self.init_side: str = ""
        self.init_cid: str = ""
        self.init_px: float = "" or 0.0
        self.init_qty: float = "" or 0.0

        # Snapshot of (entry_price, entry_qty, tp_price) on TP fill
        self._pending_snapshot: Optional[Tuple[float, float, Decimal]] = None
    ...
    def on_fill_event(self, ev: Any) -> None:
        ...
        print(f"[TPSL] on_fill_event called, init_px={self.init_px}, init_qty={self.init_qty}, event={ev!r}")

        cid  = getattr(ev, "algoClOrdId", None)
        side = getattr(ev, "side", "").upper()
        fill_px   = Decimal(str(getattr(ev, "fillPx", 0) or 0))

        if cid != self.init_cid or fill_px <= 0:
            #debug
            print(f"[TPSL] on_fill_event: self.entry_cid={self.init_cid}, is not, cid={cid}, - ignoring event , fill_px={fill_px},")
            return

        is_tp = (
            (self.init_side == "long"  and side == "SELL")
            or
            (self.init_side == "short" and side == "BUY")
        )
        if not is_tp:
            #debug
            print(f"[TPSL]2 on_fill_event: ignoring event for side={side}, init_side={self.init_side}")
            return
