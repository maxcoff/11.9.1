...
class PositionSnapshot:
    ...
    async def sync_from_rest(self) -> None:
        """Принудительно обновить из REST при старте/реконнекте."""
    ...
    async def _fetch_and_store(self) -> None:
        ...
        #logger.debug(f"[PositionSnapshot] REST positions: L={long_q} ,{long_px}, S={short_q}, {short_px}")
        # ровно одна нога
        init_side = None
        init_qty = 0.0
        init_px = 0.0
        ...
        async with self._lock:
            if init_side and init_px > 0:
                #logger.debug(f"[PositionSnapshot] REST positions: init_side={init_side}, init_qty={init_qty}, init_px={init_px}")
        ...
        # debug
        logger.debug(
            f"[PositionSnapshot] updated: {self._snapshot}",
            extra={"mode": "SNAPSHOT"}
        )
    # ----------------------------------------------------------
