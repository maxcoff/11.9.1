...
from core.task_tracker import create_tracked_task
...
class TpslMonitor:
    def __init__(self, rest, ws_monitor):
        ...
        self.last_tp_fill_px: float = 0.0 or 0.0
    ...
    def start(self) -> None:
        ...
        if not self._task or self._task.done():
            self._task = create_tracked_task(self._loop(), "TPSL monitor")
    ...
    def on_fill_event(self, ev: Any) -> None:
        ...
        #logger.debug(f"[TPSL-DEBUG] cid={cid} fill_px={fill_px} entry_cid={self.entry_cid}")
    ...
    async def bootstrap(self) -> None:
        """
        Принудительно выставляет TP/SL и заполняет entry_cid,
        если при старте уже есть обе ноги.
        """
        lq, lp, sq, sp = await self._get_state()
        #logger.debug(f"[TPSL-BOOTSTRAP] lq={lq} sq={sq}")
        if lq <= 0 or sq <= 0:
            return  # одна нога или пусто – ничего не делаем

        # Определяем исходную и хедж-сторону
        ...
        while not self._stopped:
            try:
                ...
                try:
                    ...
                    # debug output
                    #logger.debug(f"[TPSL] fetched {len(algos)} pending algos", extra={"mode": "TPSL"})
                except asyncio.TimeoutError:
                ...
                # debug 
                # Забираем реальные значения из current
                raw_init_px  = current.get(cid_init,  0)
                raw_hedge_px = current.get(cid_hedge, 0)

                # Приводим к числам (float или Decimal)
                # Вариант с float:
                init_px_api  = float(raw_init_px)
                hedge_px_api = float(raw_hedge_px)

                # Вычисляем относительные расхождения
                diff_init  = abs(init_px_api  - tp_init)  / tp_init  if tp_init  else float('inf')
                diff_hedge = abs(hedge_px_api - tp_hedge) / tp_hedge if tp_hedge else float('inf')

                #logger.debug("🔍 [TPSL] mismatch debug:\n"f"  init: api={init_px_api!r}, expected={tp_init!r}, "f"diff={diff_init:.4%}\n"f"  hedge: api={hedge_px_api!r}, expected={tp_hedge!r}, "f"diff={diff_hedge:.4%}\n"f"  threshold={self.threshold:.4%}")
