...
class TpslMonitor:
    ...
    def on_fill_event(self, ev: Any) -> None:
        ...
        if cid != self.entry_cid or px <= 0:
            #debug
            print(f"[TPSL] on_fill_event: self.entry_cid={self.entry_cid}, is not, cid={cid}, px={px}, - ignoring event")


                        print(f"[TPSL]1 on_fill_event: ignoring event for cid={cid}, side={side}, px={px}")
            return
