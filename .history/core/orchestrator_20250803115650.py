                ...
                if self.hedged and not (long_q > 0) ^ (short_q > 0):                    
                    if self.prehedge_task :
                        ...
                        try:
                            ...
                            logger.info("prehedge_task fully cancelled", extra={"mode":"PREHEDGE"})
                        if self.prehedge_task:
                            logger.info("PRE-HEDGE task on", extra={"mode": "PREHEDGE"})
                        if self.prehedge_task.done():
                            logger.info("PRE-HEDGE task cancelled", extra={"mode": "PREHEDGE"})
