...
class WSMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self.mark_prices: dict[tuple[str, str], float] = {}
    ...
    async def _process_positions(self, data: list) -> None:
        async with self._lock:
            for itm in data:
                ...
                mark = float(itm.get("markPx") or 0)
                self.positions[(inst, side)]    = pos
                self.entry_prices[(inst, side)] = avg
                self.mark_prices[(inst, side)] = mark
    ...
    def remove_ticker_listener(self, fn: Callable[[float], None]) -> None:
        """
        Unsubscribe from real-time ticker updates.
        """    
        self._ticker_listeners.remove(fn)
    ...
    async def get_mark_price_for_position(self, inst: str, side: str) -> float:
        async with self._lock:
            return self.mark_prices.get((inst, side), 0.0)
