...
class PreHedgeWatcher:
    ...
    async def run(self):    
        ...
        if self._loss_task and not self._loss_task.done():
            self._loss_task.cancel()
            logger.debug("[PreHEDGE] cancelled old loss_task")
    ...
    def _on_ticker(self, last_px: float):
        #logger.debug(f"[PreHEDGE] on_ticker got price={last_px}, entry={self.entry_price}, side={self.side}")
        ...
        pnl = direction * (last_px_dec - entry) / entry
        
        # учёт шорта
        #if self.side == "short":
        #    pnl = -pnl
        logger.debug(f"pnl={pnl:.4f}, threshold={-self.threshold:.4f}, cond={pnl < -self.threshold}")

        # если ниже −threshold → триггерим        
        if pnl < -Decimal(str(self.threshold)):
            logger.info(f"🚨 loss trigger hit: pnl={pnl:.4f}")
            self._loss_evt.set()
        #if pnl < -self.threshold: 
            #logger.debug(f"[PreHEDGE] ticker: pnl={pnl:.4f}, threshold={-self.threshold:.4f}, loss_evt_set={self._loss_evt.is_set()}")

        
            if self.spike_filter_delay > 0:
                # запускаем только одну отложенную проверку
                if not self._loss_task or self._loss_task.done():
                    logger.debug("[PreHEDGE] creating _loss_task from %s", inspect.stack()[1].function)
                    self._loss_task = asyncio.create_task(
                        self._delayed_loss_check()
                    )
            else:
                if not self._loss_evt.is_set():                    
                    logger.info(f"[PreHEDGE] loss trigger hit: pnl={pnl:.4f} (threshold={self.threshold})")
                    # сразу триггерим
                    self._loss_evt.set()
