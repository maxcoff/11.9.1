...
class OrderClient:
    def __init__(self, rest_client, ws_monitor, logger=logger):
        ...
        self.size     = float(get("ORDER_SIZE") or "0")
        self.spread   = float(get("SPREAD") or "0")
        self.interval = float(get("ORDER_INTERVAL") or "1")
    ...
    async def run(self):
        ...
        while True:
            lq = await self.ws_monitor.get_position(self.inst, "long")
            sq = await self.ws_monitor.get_position(self.inst, "short")
            if lq == 0 and sq == 0:                
                break
            await asyncio.sleep(self.interval)

        # 1) снимаем все старые лимитники
        await self.cancel_all_orders(self.inst)
        ...
        buy_px  = round(mid * (1 - self.spread/2), 8)
        sell_px = round(mid * (1 + self.spread/2), 8)
        ...
        filled = await self._wait_for_fill(timeout=self.interval)
        if not filled:
            self.logger.info(f"🕐 [OrderMode] no fill after {self.interval}s, retry", extra={"mode":"ORDER"})
            # чистим и перезапускаем цикл
            for oid in self.ord_ids:
                await self.cancel_order(self.inst, oid)
            return await self.run()

        # 5) post-fill cleanup
        await self._on_filled(filled, self.interval)
        self.logger.info(f"✅ [OrderMode] filled {filled}, ORDER mode turn off", extra={"mode":"ORDER"})
        return
        ...
        while time.time() < deadline:
            ...
            for oid in self.ord_ids:
                ...
                if status and status.lower().startswith("f"):
                    return oid
            #проверяем позиции (любая «чужая» или ручная)
            l = await self.ws_monitor.get_position(self.inst, "long")
            s = await self.ws_monitor.get_position(self.inst, "short")
            if l or s > 0:
                # появилась или изменилась позиция, инициализированная не через наш oid
                return "black_sheep"
        return None
