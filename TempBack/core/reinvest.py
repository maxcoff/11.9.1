...
class ReinvestManager:
    ...
    async def _do_reinvest(self, snapshot: Tuple[Decimal, Decimal, Decimal, str]) -> None:
        ...
        percent = Decimal(str(self.reinvest_p))
        use_amount = realized_pnl * percent
        logger.info(
            f"[REINVEST] realized={realized_pnl} * percent = {percent} -> use={use_amount}", extra={"mode": "REINVEST"}, )

        # 1. Остановить tpsl-цикл, чтобы он не мешал
        await self.task_manager.stop("tpsl")
