...
async def main():
    ...
    # 5) WS-монитор (подписываем только приватные каналы)
    ...
    # 6) Торговые клиенты
    order_client  = OrderClient(rest_client, ws_monitor)
    hedge_manager = HedgeManager(rest_client, ws_monitor)
    ...
    try:
        ...
        # корректно останавливаем WS и REST
        await ws_monitor.disconnect()
        await rest_client.close()
        # session уже закрывается внутри rest_client.close()
